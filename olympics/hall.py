...
<p class='muted'>Only finished runs are listed. Generated {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')} from {len(recs)} event records. Cost and token columns are reported per stack in the repository results, at public API list prices.</p><footer style='margin-top:40px;padding-top:16px;border-top:1px solid var(--rule);font:12px/1.5 ui-monospace,Menlo,monospace;color:#6e665d'>Browser Use Olympics by <a href='https://almond.build/'>Almond</a> · the site, the clock and the Hall of Fame run on Almond · <a href='./'>Take part</a> · <a href='https://github.com/eriestra/browser-use-olympics'>source</a></footer>
</div></body></html>"""
